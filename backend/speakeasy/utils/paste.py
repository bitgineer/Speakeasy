...
def replace_active_text(text: str, keep_in_clipboard: bool = False) -> None:
    """
    Replace the current text in the active window with new text.
    Sends Ctrl+A to select all, then pastes via clipboard.
    """
    try:
        from .clipboard import backup_clipboard, restore_clipboard, set_clipboard
        ...
        if set_clipboard(text):
            ...
            logger.warning("Clipboard set failed, using type_text fallback")
            select_all_and_paste()
            type_text(text)
    except Exception as e:
        logger.error(f"Live replace failed: {e}", exc_info=True)
