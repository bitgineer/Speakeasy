...
def _setup_live_transcription(loop: asyncio.AbstractEventLoop | None = None) -> None:
    ...
    if settings and settings.live_transcription:
...
def _preload_model_frameworks(model_type: str | None = None) -> None:
    ...
    These imports take 30-75 seconds on first load (matplotlib font cache,
    pytorch_lightning, CUDA JIT). Running them in background threads during
    startup means the user's first model load only pays for model weight
    initialization (~23s for NeMo, ~5s for Whisper), not the full import chain.

    Args:
        model_type: The configured model type. Used to prioritize which
                    framework to pre-import first. If None, imports NeMo
                    (most common) and faster-whisper.
    """
    import threading

    def _import_nemo():
        """Pre-import NeMo ASR chain. Takes ~30-75s on first import."""
        try:
            logger.info("Pre-importing NeMo ASR framework (background)...")
    ...
    def _import_faster_whisper():
        """Pre-import faster-whisper/CTranslate2. Takes ~2-5s."""
        try:
    ...
    # NeMo is the heaviest — always pre-import it unless we know it's not needed
    if model_type in (None, "parakeet", "canary"):
        threading.Thread(target=_import_nemo, daemon=True, name="nemo-preload").start()

    if model_type in (None, "whisper"):
        threading.Thread(target=_import_faster_whisper, daemon=True, name="whisper-preload").start()
...
async def lifespan(app: FastAPI):
    ...
    # Pre-import heavy ML frameworks in background threads
    # NeMo alone takes ~75s to import (matplotlib font cache + pytorch_lightning + cuda setup)
    # Doing this now in the background means model loads only pay the model-init cost (~23s)
    _preload_model_frameworks(settings.model_type if settings.model_name else None)

    # Auto-load model if configured (now runs after pre-import or in parallel)
    if settings.model_name:
        try:
            ...
            def _auto_load_and_setup():
                ...
                _setup_live_transcription(main_loop)

            threading.Thread(target=_auto_load_and_setup, daemon=True).start()
        except Exception as e:
            logger.warning(f"Failed to auto-load model: {e}")

    # Don't call _setup_live_transcription here — model thread handles it
...
async def _get_export_records(body: ExportRequest) -> list:
    ...
    records, _, _ = await history.list(limit=10000, offset=0, search=body.search)
...
async def history_export_post(body: ExportRequest):
    ...
    records = await _get_export_records(body)
    content, filename, content_type = export_service.export(
...
async def models_load(request: Request, body: ModelLoadRequest):
    ...
    try:
        ...
        if "cancelled" in str(e).lower():
            await _broadcast_model_load_error(body.model_name, "", cancelled=True)
            raise HTTPException(status_code=499, detail="Download cancelled")
        await _broadcast_model_load_error(body.model_name, str(e))
        raise HTTPException(status_code=500, detail=str(e))
